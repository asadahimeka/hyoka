          ...
          <mu-table v-show="docked&&adding" fixedHeader :showCheckbox="false" :selectable="false" ref="table">
            ...
            <mu-tbody>
              ...
              <mu-tr>
                ...
                <mu-td>
                  <mu-text-field v-model="teacherA" fullWidth />
                </mu-td>
                <mu-td class="tac">
          ...
          <mu-tbody v-for="(item,index) in evas" :key="item.node.id">
            ...
            <transition name="td1" mode="out-in">
              <mu-tr v-show="editing!=index">
                <mu-td>{{item.node.cno}}</mu-td>
                <mu-td>{{item.node.name}}</mu-td>
                <mu-td>{{item.node.teacher}}</mu-td>
                <mu-td class="tac">
            ...
            <transition name="td1" mode="out-in">
              <mu-tr v-show="editing==index">
                ...
                <mu-td>
                  <mu-text-field v-model="teacherE" fullWidth/>
                </mu-td>
                <mu-td class="tac">
  ...
  methods: {
    async getIndexs() {
      try {
        ...
        this.evas = res.data.courses.edges
        this.loading = false
        this.$loading('close')
      } catch (err) {
        console.error(err)
    ...
    async doneAdd() {
      if (!this.evnoA || !this.evnameA || !this.teacherA) {
        this.$popup('请输入内容')
        return
      }
      try {
    ...
    async doneEdit(index, id) {
      ...
      try {
        this.evas[index].node = {
          ...this.evas[index].node,
          name: this.evnameE,
          teacher: this.teacherE
        }
        let res = await this.$api.editCourse(id, this.evas[index].node)
        let { error } = res.data.CourseEdit
        if (error) {
          this.$popup('编辑出错')
          return
        }
    ...
    del(id, index) {
      this.$dialog({
        ...
        submitFn: async () => {
          try {
            let res = await this.$api.del(id, 'course')
